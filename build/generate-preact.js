...
function buildCreateIconSource () {
  return `import * as Preact from 'preact/compat'
...
function renderNodes (nodes) {
  return nodes.map((node, index) => {
    ...
    return Preact.createElement(node.tag, props, ...children)
  })
}

export function createIcon (iconName, iconNode) {
  const component = Preact.forwardRef(function MeteorIcon (props, ref) {
    const {
      size = 24,
      strokeWidth = 2,
      color = 'currentColor',
...
async function generatePreactPackage () {
  ...
  await fs.writeFile(new URL('./index.js', PREACT_DIR), buildIndexSource(iconNames), 'utf8')
  await fs.writeFile(new URL('./index.d.ts', PREACT_DIR), buildIndexTypesSource(iconNames), 'utf8')

  const rootPath = path.normalize(ROOT_DIR.pathname)
  console.log(`Preact package generated (${iconNames.length} icons) at ${rootPath}`)
}
