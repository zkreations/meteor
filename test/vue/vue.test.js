...
import { createIcon } from '../../packages/vue/src/create-icon.js'
import AlarmClock from '../../packages/vue/src/icons/alarm-clock.js'

import ArrowRight from '../../packages/vue/src/icons/arrow-right.js'
...
describe('vue icons', () => {
  ...
  it('renders minimal svg attributes when minimal is enabled', () => {
    const { container } = render(AlarmClock, {
      props: {
        minimal: true,
      },
      attrs: {
    ...
    expect(svg.getAttribute('class')).toContain('i i-alarm-clock')
  ...
  it('sets the expected component name', () => {
    expect(AlarmClock.name).toBe('alarm-clock')
    expect(ArrowRight.name).toBe('arrow-right')
  })
